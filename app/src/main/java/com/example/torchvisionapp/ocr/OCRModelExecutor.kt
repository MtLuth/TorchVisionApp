...
class OCRModelExecutor(context: Context, private var useGPU: Boolean = false) : AutoCloseable {
    ...
    private val recognitionResult: ByteBuffer   // the variable to store the result of text recognition
    private val detectionInterpreter: Interpreter   // the instance of the model Text detection => this instance will predict bounding boxes
    private val recognitionInterpreter: Interpreter   // the instance of the model Text recognition => this instance will predict the text for each bounding box
    ...
    init {
        try {
            if (!OpenCVLoader.initDebug()) throw Exception("Unable to load lib OpenCV")
            else Log.d(TAG, "Lib OpenCV loaded")
        } catch (e: Exception) {
            val exceptionLog = "Failed: ${e.message}"
            Log.d(TAG, exceptionLog)
        }

        // load the text detection model from model file ["app/src/main/assets/text_detection.tflite"]
        detectionInterpreter = getInterpreter(context, textDetectionModel, useGPU)

        // load the text recognition from model file ["app/src/main/assets/text_recognition.tflite"]
    ...
    private fun detectTexts(data: Bitmap) {
        val detectionTensorImage = ImageUtils.bitmapToTensorImageForDetection(
            data, detectionImageWidth, detectionImageHeight, detectionImageMeans, detectionImageStds
        )

        val detectionInputs =
            arrayOf(detectionTensorImage.buffer.rewind())   // the variable to store the input of Text detection
        val detectionOutputs: HashMap<Int, Any> =
            HashMap<Int, Any>()   // the variable to store the output (result) of Text detection
        ...
        // run Text detection to predict the boundind boxes
        ...
        for (y in 0 until transposeddetectionScores[0][0].size) {
            ...
            for (x in 0 until transposeddetectionScores[0][0][0].size) {
                ...
                val textDetection = RotatedRect(
                    center, Size(w.toDouble(), h.toDouble()), (-1 * angle * 180.0 / Math.PI)
                )
    ...
    // define func to predict the texts by using Text recognition model => stage 2: Text recognition
    private fun recognizeTexts(
        data: Bitmap, boundingBoxesMat: MatOfRotatedRect, indicesMat: MatOfInt
    ): Bitmap {
        ...
        for (i in indicesMat.toArray()) {
            ...
            targetVertices.add(
                Point(
                    (recognitionImageWidth - 1).toDouble(), (recognitionImageHeight - 1).toDouble()
                )
            )
            ...
            val targetVerticesMat = MatOfPoint2f(
                targetVertices[0], targetVertices[1], targetVertices[2], targetVertices[3]
            )
            ...
            val recognitionBitmap = ImageUtils.createEmptyBitmap(
                recognitionImageWidth, recognitionImageHeight, 0, Bitmap.Config.ARGB_8888
            )
            matToBitmap(recognitionBitmapMat, recognitionBitmap)

            val recognitionTensorImage = ImageUtils.bitmapToTensorImageForRecognition(
                recognitionBitmap,
            ...
            for (k in 0 until recognitionModelOutputSize) {
                ...
                if (alphabetIndex in 0..alphabets.length - 1) recognizedText =
    ...
    private fun getInterpreter(
        context: Context, modelName: String, useGpu: Boolean = false
    ): Interpreter {
    ...
    companion object {
        ...
        private const val textDetectionModel = "models/ocr/text_detection.tflite"
        private const val textRecognitionModel = "models/ocr/text_recognition.tflite"
